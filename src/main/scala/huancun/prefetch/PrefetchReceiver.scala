...
class PrefetchReceiver()(implicit p: Parameters) extends HuanCunModule with HasPrefetchParameters {
  val io = IO(new PrefetchIO())
  // just ignore train reqs
  io.train.ready := true.B
  io.resp.ready := true.B

  io.req.bits.tag := parseFullAddress(io.recv_addr.bits, setBits, fullTagBits)._1
  io.req.bits.set := parseFullAddress(io.recv_addr.bits, setBits, fullTagBits)._2
