...
class Slice()(implicit p: Parameters) extends HuanCunModule {
  ...
  dataStorage.io.sinkD_waddr.bits.set := sinkD_wset(
    associativePolicy.get_func_id(sinkD.io.bs_waddr.bits.way)
  )
  ...
  dataStorage.io.sourceD_raddr.bits.set := sourceD_rset(
    associativePolicy.get_func_id(sourceD.io.bs_raddr.bits.way)
  )
  ...
  dataStorage.io.sourceD_waddr.bits.set := sourceD_wset(
    associativePolicy.get_func_id(sourceD.io.bs_waddr.bits.way)
  )
...
//  dataStorage.io.sinkC_waddr <> ctrl_arb(sinkC.io.bs_waddr, ctrl.map(_.io.bs_w_addr))
  ...
  dataStorage.io.sourceC_raddr.bits.set := sourceC_rset(
    associativePolicy.get_func_id(sourceC_arb.bits.way)
  )
  ...
  dataStorage.io.sinkC_waddr.bits.set := sinkC_wset(
    associativePolicy.get_func_id(sinkC_arb.bits.way)
  )
