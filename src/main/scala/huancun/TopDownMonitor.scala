...
class TopDownMonitor()(implicit p: Parameters) extends HuanCunModule {
   ...
   */
  for (
    ((hartId, pAddr), addrMatch) <- cacheParams.hartIds.zip(io.debugTopDown.robHeadPaddr).zip(io.debugTopDown.addrMatch)
  ) {
    val addrMatchVec = io.msStatus.zipWithIndex.map {
      case (slice, i) =>
        slice.map { ms =>
          val msBlockAddr =
            if (bankBits == 0) Cat(ms.bits.tag, ms.bits.set)
  ...
  def dirResultMatchVec(cond: DirResult => Bool): IndexedSeq[Bool] = {
    io.dirResult.map { r =>
      RegNext(r.valid, false.B) && r.bits.replacerInfo.channel === 1.U && cond(r.bits)
    }
  }
