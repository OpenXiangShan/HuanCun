...
class SRAM_Array_1P_MCP(depth: Int, width: Int, maskSegments: Int, mbist:Boolean) extends SRAM_Array_1P(depth, width, maskSegments, mbist)

class SRAM_Array_2P(depth: Int, width: Int, maskSegments: Int, mbist:Boolean) extends SRAM_Array(mbist)  {
...
(
  ...
  val mbistGroupsNto1 = List.tabulate(way,mbistNodeNumForEachWay)((widx:Int,nidx:Int) => mbistNodesNto1(widx * mbistNodeNumForEachWay + nidx))
  ...
  val finalReadSetIdx = if(mbist) Mux(mbistFuncSel, mbistAddrRead, setIdx) else io.r.req.bits.setIdx
