...
class Directory(implicit p: Parameters) extends BaseDirectory[DirResult, DirWrite, TagWrite] {
  ...
  def invalid_way_sel(metaVec: Seq[DirectoryEntry], repl: UInt, waymask: UInt) = {
    val invalid_vec = metaVec.map(_.state === MetaData.INVALID)
    val has_invalid_way = (Cat(invalid_vec) & Reverse(waymask)).orR()
    val way = ParallelPriorityMux(invalid_vec.zipWithIndex.map(x => (x._1 & waymask(x._2)) -> x._2.U(wayBits.W)))
    (has_invalid_way, way)
  }
