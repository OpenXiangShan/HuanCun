...
//  oc.set := req.set
  ...
  val replace_release_set = associativePolicy.get_unhashed_index(meta.tag, req_hash_set)
  oc.set := Mux(req.fromB, req.set, replace_release_set(associativePolicy.choose_a_hash_func(meta.way)))
  oc.param := Mux(
    req.fromB,
    MuxLookup(
